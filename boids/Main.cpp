...
template<typename F>
ChunkedFlock::ChunkedFlock(F dna, float sWeight, float cWeight, float aWeight, const int& splits, const sf::Vector2u& dimensions) :
...
template<typename F>
Flock::Flock(F dna, float sWeight, float cWeight, float aWeight) : w(sWeight, cWeight, aWeight) {
